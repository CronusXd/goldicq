...
jQuery.fn.floatdiv=function(location){
    ...
    return this.each(function(){
        ...
        if(location==undefined||location.constructor==String){
            switch(location){
                ...
                case("middlebottom"):
                    ...
                    break;
                case("leftmiddle"):
                    loc={left:"0px",top:windowHeight/2.5-jQuery(this).height()/2+"px"};
        ...
        if(isIE6){
            if(top>=0){
                wrap=jQuery("<div style=\"top:expression(documentElement.scrollTop+documentElement.clientHeight /2-this.offsetHeight/2);\"></div>");
            }
            else{
                wrap=jQuery("<div style=\"top:expression(documentElement.scrollTop+documentElement.clientHeight-this.offsetHeight);\"></div>");
            }
        }
        jQuery("body").append(wrap);wrap.css(loc).css({position:"fixed",z_index:"999"});
        if(isIE6){
